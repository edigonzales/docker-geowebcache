...
#RUN  echo "deb http://httpredir.debian.org/debian jessie non-free" >> /etc/apt/sources.list     && apt-get update     && apt-get install -y apt-utils     && apt-get install -y libjai-core-java libjai-imageio-core-java     && rm -rf /var/lib/apt/lists/
...
RUN wget -O /tmp/geowebcache.zip https://sourceforge.net/projects/geowebcache/files/geowebcache/1.17.3/geowebcache-1.17.3-war.zip/download \
